...
use super::{
    Digest, RevocationTailsAccessor, RevocationTailsGenerator, Sha256, Tail, UrsaCryptoError,
    UrsaCryptoErrorKind,
};
...
impl TailsReader {
    pub fn new<TR: TailsReaderImpl + 'static>(inner: TR) -> Self {
        Self {
...
pub trait TailsReaderImpl: std::fmt::Debug + Send {
    fn hash(&mut self) -> IndyResult<Vec<u8>>;
...
impl TailsFileReader {
    ...
    pub fn open(&mut self) -> IndyResult<()> {
