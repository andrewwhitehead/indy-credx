...
impl Query {
    fn optimise(self) -> Query {
        match self {
            ...
            Query::And(mut suboperators) => {
                if suboperators.len() == 1 {
                    suboperators.remove(0)
                } else {
                    Query::And(suboperators)
                }
            }
            Query::Or(mut suboperators) => {
                if suboperators.len() == 1 {
                    suboperators.remove(0)
                } else {
    ...
    pub fn clean(&self) -> Option<&Query> {
        // TODO: improve it to handle nested
        match self {
...
fn parse_operator(key: String, value: serde_json::Value) -> Result<Query, &'static str> {
    match (key.as_str(), value) {
        ...
        (_, serde_json::Value::String(value)) => Ok(Query::Eq(key, value)),
